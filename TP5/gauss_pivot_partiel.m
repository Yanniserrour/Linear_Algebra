...
%resolution a travers la méthode de gauss avec choix de pivot partiel
for k = 1 : n-1 
    [val_max, indice_max] = max(abs(A(k:n,k))); %prendre l'indice de la val
    l = indice_max + k-1; %pour revenir a la matrice général 
    if l~=k % verifier si l'echange est obligatoir 
        A([k l],:) = A([l k],:);
        b([k l]) = b([l k]);
    end
    for i = k + 1 : n 
